...
class VectorEvaluate(object):
    ...
    def interp_fixed_num(self, vector: NDArray, num_pts: int) -> NDArray:
    ...
    def interp_fixed_dist(self, vector: NDArray, sample_dist: float) -> NDArray:
    ...
    def _evaluate_single(
        ...
        tp_fp_list = instance_match(
            pred_lines, scores, gt_lines, thresholds, metric
        )  # (M, 2)
        tp_fp_score_by_thr = {}
    ...
    def evaluate(
        self, result_path: str, metric: str = "chamfer", logger: Optional[Logger] = None
    ) -> Dict[str, float]:
        ...
        for label in self.id2cat.keys():
            ...
            fn = partial(
                self._evaluate_single, thresholds=self.thresholds, metric=metric
            )
        ...
        table = prettytable.PrettyTable(
            ...
            + [f"AP@{thr}" for thr in self.thresholds]
            + ["AP"]
        )
        for label in self.id2cat.keys():
            table.add_row(
                [
                    ...
                    *[
                        round(result_dict[self.id2cat[label]][f"AP@{thr}"], 4)
                        for thr in self.thresholds
                    ],
