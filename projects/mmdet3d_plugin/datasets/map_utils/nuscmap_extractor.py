...
from .utils import (get_drivable_area_contour, get_ped_crossing_contour,
                    split_collections)
...
class NuscMapExtractor(object):
    ...
    def __init__(self, data_root: str, roi_size: Union[List, Tuple]) -> None:
        ...
        for loc in self.MAPS:
            self.nusc_maps[loc] = NuScenesMap(dataroot=data_root, map_name=loc)
            self.map_explorer[loc] = NuScenesMapExplorer(self.nusc_maps[loc])

        # local patch in nuScenes format
        self.local_patch = box(
            -roi_size[0] / 2, -roi_size[1] / 2, roi_size[0] / 2, roi_size[1] / 2
        )
    ...
    def get_map_geom(
        ...
        lanes = self.map_explorer[location]._get_layer_polygon(patch_box, yaw, "lane")
