...
def split_collections(geom: BaseGeometry) -> List[Optional[BaseGeometry]]:
    ...
    assert geom.geom_type in [
        ...
        "Polygon",
        "GeometryCollection",
    ], f"got geom type {geom.geom_type}"
...
def get_drivable_area_contour(
    drivable_areas: List[Polygon], roi_size: Tuple
) -> List[LineString]:
...
def get_ped_crossing_contour(
    polygon: Polygon, local_patch: box
) -> Optional[LineString]:
