...
class VectorizeMap(object):
    ...
    def permute_line(self, line: np.ndarray, padding=1e5):
        ...
        if is_closed:
            ...
            # padding
            padding = np.full(
                [permute_num * 2 - 2, num_points, self.coords_dim], padding
            )
    ...
    def __call__(self, input_dict):
        ...
        if self.permute:
            ...
            input_dict["gt_map_pts"] = np.array(gt_map_pts, dtype=np.float32).reshape(
                -1, 2 * (self.sample_num - 1), self.sample_num, self.coords_dim
            )
        else:
