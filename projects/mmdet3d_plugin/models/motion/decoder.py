...
class SparseBox3DMotionDecoder(SparseBox3DDecoder):
    ...
    def decode(
        ...
        for i in range(bs):
            ...
            output.append({"trajs_3d": traj.cpu(), "trajs_score": traj_cls.cpu()})
...
class HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        classification, final_planning = self.select(
            det_output, motion_output, classification, prediction, data
        )
    ...
    def rescore(
        ...
        ego_box = (
            ego_box[:, None, None]
            .repeat(1, num_anchor, num_motion_mode, 1, 1, 1)
            .flatten(0, -2)
        )
        motion_box = (
            motion_box.unsqueeze(3).repeat(1, 1, 1, num_ego_mode, 1, 1).flatten(0, -2)
        )

        ego_box[0] += offset * torch.cos(ego_box[6])
        ego_box[1] += offset * torch.sin(ego_box[6])
        col = check_collision(ego_box, motion_box)
        col = col.reshape(bs, num_anchor, num_motion_mode, num_ego_mode, ts).permute(
            0, 3, 1, 2, 4
        )
...
def corners_in_box(boxes1, boxes2):
    ...
    collision = torch.logical_and(
        torch.logical_and(
            corners_box2[..., 0] <= H / 2, corners_box2[..., 0] >= -H / 2
        ),
        torch.logical_and(
            corners_box2[..., 1] <= W / 2, corners_box2[..., 1] >= -W / 2
        ),
    )
