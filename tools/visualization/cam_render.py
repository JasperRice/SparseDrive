...
from tools.visualization.bev_render import (CMD_LIST, MAP_SCORE_THRESH,
                                            SCORE_THRESH, color_mapping)
...
class CamRender:
    ...
    def _render_traj(
        ...
        traj_points = view_points(total_xy.T, cam_intrinsic, normalize=True)[:2, :]
        traj_points = traj_points[:2, in_range_mask]
        self.axes[j // 3, j % 3].scatter(
            traj_points[0], traj_points[1], color=color, s=s
        )
